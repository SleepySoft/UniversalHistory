...
- 无状态栏、无 Help 菜单、**无退出确认**（旧版有中文确认框——回摆项，待决策）。
...
旧版修复/变化：Toggle Orientation 真正可用（旧版对话框构造即崩溃）；修复旧版 Ctrl+R 双绑冲突；移除空壳 Help/About 与 Load Depot/Load All Records；双击编辑器与主界面联动刷新（旧版零联动）。
...
5. **Switch side** / **Remove this thread**（**移除 Thread 无确认**）；
6. **Fit to view** / **Toggle orientation**；
7. 命中事件时追加 **Edit event** / **Delete event**（Record 级右键——旧版没有）。

**Delete event**：确认框 "Confirm Delete" / `Delete '{abstract[:40]}'?`；Yes 后 `workspace.remove` + `adapter.save_file(...)` **立即落盘**，失败弹 "Save Failed"。
