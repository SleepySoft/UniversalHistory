...
- [ ] 界面任何时刻可回答「这条事件保存在哪个文件」（O2 归属可见）；
- [ ] 新建事件的所有路径都有明确目标文件，无「保存时才问」分支（O4）；
- [ ] 编辑已有事件永远写回其自身 source（O3）；
- [ ] 删除/更新不波及同 uuid 的其他 source 记录（O5）；
- [ ] 编辑器不含文件管理（O6）。

## 8. 时间轴为中心（[15-timeline-centric-editing.md](15-timeline-centric-editing.md)）
