...
| 39 | `render/timeline_view.py` `_update_hover` | **悬停提示用户感知为「没有」**：用 widget toolTip 属性被动弹出，光标须停留约 1 秒才显示，移动中永不出现（旧版是主动跟随弹窗） | **已修复（2026-09-19）**：恢复旧版行为——命中即 `QToolTip.showText` 主动弹出并跟随光标（每次移动重显，「Year N of M」进度实时刷新）；移出 item / 离开控件 / 开始拖动时 `hideText`；新增 `leaveEvent`；回归测试 `tests/render_tests/test_hover_tooltip.py`；真实平台验证弹窗即时可见 |
