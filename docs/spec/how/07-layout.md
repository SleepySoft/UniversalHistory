...
2. **轨道从贴轴侧向外编号**（2026-09-19 修正）：正侧（y0≥0）轨 0 在 y0，负侧（y1≤0）轨 0 在 y1 端——两侧都「优先靠近轴」，横纵布局行为一致（此前负侧从远端排起，横布局上方 Thread 与纵布局左侧 Thread 行为相反）；末轨 = 最外侧轨。
3. **长优先排序**：key = `(-(until-since), since)`——持续时长降序 + 开始时间升序 tie-break（旧版依赖 sort 稳定性，新版确定性）。无时间事件排最前但随后跳过不布局。
4. **单点事件虚拟像素区间**：is_point → `x0 = time_x`、`x1 = time_x + 120`（120px 卡片**左缘锚定**在事件时刻，棒棒糖设计，见 [08-painting.md](08-painting.md) §4）；持续事件 → `[since_x, until_x]`。**单点与持续以同一套区间逻辑参与统一分配**——不再固定轨 0（§8.5 决策落地；回归测试：远处三个单点应占 ≥2 条轨）。
...
9. **全量布局**：对所有事件布局（不做可见性裁剪）——继承旧版「全量重排保稳定」哲学；绘制与命中测试按可见范围裁剪（#19），布局本身保持全量。
