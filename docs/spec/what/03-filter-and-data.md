...
为减小传输尺寸，记录有一种**简化形态**：只保留 uuid、source、起止时间和 50 字符摘要（正文与完整标签不进入）。它由完整事件现场派生、不单独持久化，用于时间轴渲染、大列表和未来的网络传输（先传简化记录、按需取完整内容）。旧版把 index 做成独立概念和 .index 文件管线（且已断链），新版不沿用。
