...
- **悬停实时提示**：光标在时间轴上移动时，自绘**十字线**（过光标的水平+垂直虚线，通长）+ **跟随光标的浮动信息框**。第一行始终是光标处的日期 `(yyyy/mm/dd)`（公元前显示为 `xxxx BC/mm/dd`）；光标在事件上时追加第二行——事件摘要，单点事件附 ` : [日期]`，持续事件附 `(N/M)` 光标所在年进度与 ` : [起年 - 止年]` 区间。拖动时隐藏；可用开关关闭。行为与旧版「实时提示」逐项对齐，见 [../how/09-timeline-view.md](../how/09-timeline-view.md) §6。

## 事件显示

- **单点事件**：棒棒糖结构——轴上圆点标记精确时刻，细引线连到 120px 圆角卡片（左缘锚定在事件时刻）；缩得太远（卡片时间跨度超 2 年）时退化为时刻竖线 + 轴点，避免固定宽卡片伪装成持续事件。
