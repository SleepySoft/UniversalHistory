"""
History time handling, ported from History/Utility/HistoryTime.py.

The history time type is TICK, in seconds, with the start of the Common Era
(0001-01-01 00:00:00) as zero. CE is positive, BCE is negative; there is no
year 0. TICK is a real-world time scale; year/month/day is a man-made
calendar. BCE months and days are for reference only: the day before
0001-01-01 CE is -0001/12/31, the day before that -0001/12/30, and so on.

Features:
    1. Convert natural-language time text to TICK (no NLP, brute-force rules
       suited to time expressions in history articles):
        a) Common forms: "公元前200年", "距今1000年前", "xx年五月"; no era names.
        b) Multiple times separated by "~", "-" etc., e.g. "公元前200年 - 公元前300年"
        c) No week support.
        d) Standard datetime formats are also supported: YYYY-mm-dd HH:MM:SS

Porting notes:
- Behaviour is kept byte-for-byte compatible with the legacy parser, except
  that century expressions ("21世纪" / "公元前3世纪" / "七世纪初") now parse to
  the full 100-year interval (user decision 2026-09-18; the legacy coarse
  "世纪 -> 00" replacement is removed).
- ``decimal_year_to_tick`` was referenced by legacy HistoryRecord for
  since:/until: labels but never implemented (legacy defect #1); it is
  implemented here so those labels load instead of raising AttributeError.
"""
...
from .cn_num import text_cn_num_to_arab
from .text_utils import str_to_int, str_includes
...
# Improvement over the legacy coarse ('世纪', '00') replacement
# (legacy defect #50, user decision 2026-09-18): a century expression like
# "21世纪" / "公元前3世纪" / "七世纪初" now parses to the full 100-year
# interval instead of its end year.
CENTURY_FINDER = re.compile(r'(\d+)\s*世纪')
...
def time_text_to_ticks(time_text: str):
    ...
    *** All you need is this function ***
    ...
    """
    time_ticks = []
    sub_time_texts = _split_natural_language_time_text(time_text)
    ...
    for sub_time_text in sub_time_texts:
        ...
        if dt is not None:
            ...
            # Century expressions ("21世纪", "公元前3世纪") are intervals.
            century_ticks = _century_text_to_ticks(sub_time_text)
            if century_ticks is not None:
                time_ticks.extend(century_ticks)
                continue
            tick = _single_natural_language_time_to_tick(sub_time_text)
        if tick is not None:
            time_ticks.append(tick)
    return time_ticks


def _century_text_to_ticks(time_text: str) -> typing.Optional[list]:
    """
    Convert a century expression to its 100-year interval [since, until].

    "21世纪" -> 2001-01-01 .. 2100-12-31
    "公元前3世纪" / "前3世纪" -> -300-01-01 .. -201-12-31
    Chinese numerals are supported ("七世纪初" -> 601 .. 700).

    Returns None when the text contains no century expression, letting the
    caller fall back to single-time parsing.
    """
    arablized = text_cn_num_to_arab(time_text)
    m = CENTURY_FINDER.search(arablized)
    if not m:
        return None
    ...
    if str_includes(time_text.lower().strip(), PREFIX_BCE):
        # 公元前 N 世纪 = years -(N*100) .. -((N-1)*100 + 1); no year 0.
        since_year = -century * 100
        until_year = -(century - 1) * 100 - 1
    else:
        since_year = (century - 1) * 100 + 1
        until_year = century * 100
    return [
        date_time_data_to_tick(since_year, 1, 1),
        date_time_data_to_tick(until_year, 12, 31),
    ]


def time_str_to_datetime(text: str) -> typing.Optional[datetime.datetime]:
    """
...
def tick_to_datetime(tick: TICK) -> typing.Optional[datetime.datetime]:
    """
...
def decimal_year_to_tick(decimal_year: float) -> TICK:
    """
    Convert a decimal year (e.g. 200.5 or -200.5) to TICK.

    This function was referenced by the legacy HistoryRecord.set_label_tags()
    for since:/until: labels but never existed in the legacy HistoryTime
    module (legacy defect #1 — loading any record with since/until labels
    raised AttributeError). Implemented during the parser port.

    The integer part is the year (no year 0: 0 is nudged to +/-1); the
    fraction is a fraction of that year's length.
    """
    sign = 1 if decimal_year >= 0 else -1
    year = int(decimal_year)
    if year == 0:
        year = sign
    fraction = abs(decimal_year) - abs(year)
    return years_to_tick(year) + int(fraction * year_ticks(is_leap_year(year)))
...
def _split_natural_language_time_text(text: str) -> list:
    """
    ...
    # Match content surrounded by '[]'
    bracket_content = re.findall(r'\[.*?\]', unified_time_str)
    # Remove content surrounded by '[]'
    s = re.sub(r'\[.*?\]', '', unified_time_str)

    # Split using the separators in SEPARATOR
...
def _get_first_item_except(items: list, expect: str):
    return items[0].replace(expect, '') if len(items) > 0 else ''


def _single_natural_language_time_to_tick(time_text: str) -> typing.Optional[TICK]:
    """
    ...
    day = _get_first_item_except(DAY_FINDER.findall(arablized_str), '日')
    year = _get_first_item_except(YEAR_FINDER.findall(arablized_str), '年')
    month = _get_first_item_except(MONTH_FINDER.findall(arablized_str), '月')
...
def year_ticks(leap_year: bool) -> TICK:
    ...
    :return: seconds of 365 days if not leap year else seconds of 366 days
    """
...
def year_days(leap_year: bool) -> int:
    ...
    Get days of a year.
    :param leap_year: Is leap year or not
    :return: 366 days if leap year else 365 days
    """
...
def days_to_years(days: int) -> Tuple[int, int]:
    """
    ...
    # The 4th century of a 400-year cycle has 36525 days (year 400 is leap),
    # not DAYS_PER_100_YEARS. Clamp so the last day of the cycle
    # (e.g. 1600-12-31, 2000-12-31) stays in that century — the legacy code
    # let years_100 become 4 and spilled it into the next cycle.
    years_100 = remainder // DAYS_PER_100_YEARS
    if years_100 > 3:
        years_100 = 3
        remainder -= 3 * DAYS_PER_100_YEARS
    else:
...
def days_to_months(days: int, leap_year: bool) -> Tuple[int, int]:
    """
...
def days_to_date(days: int) -> Tuple[int, int, int]:
    """
...
def tick_to_days(tick: int) -> Tuple[int, int]:
    """
...
def tick_to_month(tick: int, leap_year: bool) -> Tuple[int, int]:
    """
...
def tick_to_years(tick: int) -> Tuple[int, int]:
    """
...
def tick_to_date(tick: int) -> Tuple[int, int, int, int]:
    """
...
def tick_to_time_data(tick: int) -> Tuple[int, int, int]:
    """
...
def tick_to_date_time_data(tick: int) -> Tuple[int, int, int, int, int, int]:
    """
